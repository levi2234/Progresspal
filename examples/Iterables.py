...
def nested_loops_2():
    for j in ltrack(range(5), taskid="Thread2_Main", total=5, host=ip):
        time.sleep(random.uniform(0.1, 0.5))
        for i in ltrack(deque([1, 2, 3, 4, 5]), taskid="Thread2_Secondary",  host=ip,total=len(deque([1, 2, 3, 4, 5]))):
            time.sleep(random.uniform(0.1, 0.5))
            for k in ltrack(np.arange(0, 5, 0.5), taskid="Thread2_Tertiary", host=ip, total=len(np.arange(0, 5, 0.5))):
                time.sleep(random.uniform(0.1, 0.5))
