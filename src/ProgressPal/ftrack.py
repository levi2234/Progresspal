...
class ftrack:
    ...
    def calllogic(self, func, *args, **kwargs):
        ...
        if self.web and self.call_count % self.tickrate == 0:
            ...
            threading.Thread(target=self.run_update, args=(func,), daemon=True).start()
        return result
    ...
    def run_update(self, func):
        ...
        with self.lock:
            update_function_progress(
                ...
                calls_per_second=self.call_count
                / (time.perf_counter() - self.first_call_time),
                host=self.host,
                port=self.port,
            )
